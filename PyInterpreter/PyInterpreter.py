...
class REPL(Singleton):
  ...
  def onConsoleInput(self, inputText, resetGlobals):
    result = 0
    try:
      result = self.run_script(inputText, resetGlobals)
    except:
      log("error")
    ...
    if self.lastCode:
      temp = self.lastCode
      self.lastCode = None
      self.onConsoleInput(temp, False, resetGlobals)
      
  def run_script(self, code, resetGlobals):
    # run script
    ...
    try:
      globalValues = {} if resetGlobals else self.globalValues
      self.old_stdout = sys.stdout
      self.redirected_output = sys.stdout = StringIO()
      try:
        print eval(code, globalValues)
      except:
        exec(code, globalValues)
...
# CLASS : InputPopup
#---------------------#
class InputPopup(Singleton):
  def show(self, *args, **kargs):
    text = str(args[0]) if len(args) > 0 else ""
    #  rowser layout
    self.inputLayout = BoxLayout(orientation="vertical", pos_hint={"top":0.8}, size_hint=(1,0.4))
    self.inputText = TextInput(text="input filename", multiline = False, padding_y="15dp", font_name=defaultFont, size_hint=(1, 1))
    self.inputLayout.add_widget(self.inputText)
    self.inputText.size = (W, self.inputText.minimum_height)
    self.inputText.text = ""
    self.inputText.bind(on_text_validate = self.onInputText)
    
    self.popup = Popup(title = text, content=self.inputLayout, auto_dismiss=True, size_hint=(1, None), height="200dp")
    self.popup.open()
    import time
    time.sleep(3.0)
    
  def onInputText(self, inst):
    print inst.text
...
class PyInterpreter(Singleton):
  ...
  def setMode(self, mode):
    ...
    if mode == szConsole:
      ...
      self.fileBrowser.showOpenLayout(self)
    elif mode == szFileBrowserSaveAs:
      self.fileBrowser.showSaveAsLayout(self)
      
  def callback_open_file(self, filename):
    self.editorLayout.open_file(filename)
    
  def callback_save_as(self, filename):
    self.editorLayout.save_as(filename)
    
  def callback_filebrowser_closed(self):
    self.setMode(szEditor)
