...
kivy_log_file = "kivy_log.txt"
if os.path.exists(kivy_log_file):
  os.remove(kivy_log_file)
  
class myLogger:
  def write(self, data):
    f = open(kivy_log_file, "a")
    f.write(data)
    f.close()
sys.stderr = myLogger()
...
def loading_postprocess():
  ...
  global gResViewer
  gResViewer = ResourceViewer.instance()
  
#---------------------#
# Global variable
#---------------------#
defaultFont = kivy.resources.resource_find(os.path.join("fonts", "DroidSansMonoDotted.ttf"))
...
class DebugPrint(Widget, Singleton):
  ...
  def init(self, bShow):
    ...
    self.btn_resViewer = Button(text="Resource Viewer", size_hint_x=3)
    self.btn_resViewer.bind(on_release=self.toggleResViewer)
    self.btn_layout.add_widget(self.btn_resViewer)
  ...
  def toggleResViewer(self, *args):
    if gResViewer.opened:
      gResViewer.closeWidget()
    else:
      gResViewer.openWidget()
  ...
  def toggleDebug(self, *args):
    if self.debugLabel in self.children:
      ...
      self.btn_layout.remove_widget(self.btn_resViewer)
...
class MyRoot(App, Singleton):
  ...
  def getClipboard(self, mimetype='text/plain'):
    ...
    try:
      ...
      primary_clip = clippy and clippy.getPrimaryClip()
      if primary_clip:
  ...
  def build(self):
    ...
    # check clipboard
    log(self.getClipboard())
    try:
      log(PythonActivity._clipboard.getText())   
    except:
      self.useAlternativePaste = True
      log("useAlternativePaste")
...
class StateItem(object):  
  ...
  def __setattr__(self, key, value):
    if key not in ("stateMgr", "onEnter", "onUpdate", "onExit"):
      raise AttributeError("You must override one of onEnter, onUpdate, onExit.")
    super(StateItem, self).__setattr__(key, value)
